...
mode = 'val'
if mode == 'train':
    dataset = sio.loadmat(os.path.join('data', 'mat_data', 'generated_data', 'train_generative_modeled_data.mat'))
elif mode == 'val':
    dataset = sio.loadmat(os.path.join('data', 'mat_data', 'generated_data', 'val_generative_modeled_data.mat'))
elif mode == 'train_extended':
    try:
        dataset = sio.loadmat(os.path.join('data', 'mat_data', 'generated_data', 'train_generative_modeled_data_extended.mat'))
    except NotImplementedError:
        dataset = {}
        with h5py.File(os.path.join('data', 'mat_data', 'generated_data', 'train_generative_modeled_data_extended.mat'), 'r') as f:
            for k, v in f.items():
...
pdb.set_trace()
if mode == 'train_extended':        
    with open(os.path.join('data', f'train_set_arun_generative_modeled_extended.pkl'), 'wb') as f:
        pickle.dump(save_dict,f)
else:
    with open(os.path.join('data', f'{mode}_set_arun_generative_modeled.pkl'), 'wb') as f:
        pickle.dump(save_dict,f)
